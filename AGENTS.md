# AGENTS.md - Strength Save (zasady pracy nad projektem)
...
- **Native-first discovery:** zanim powstanie własny most, workaround WebView albo
  nowa obsługa funkcji systemowej, sprawdź aktualną oficjalną listę pluginów
  Capacitor i zgodność z używaną wersją core. Jeśli plugin istnieje, udokumentuj
  decyzję: użyć / odrzucić / odłożyć, wraz z zachowaniem web/iOS/Android, privacy,
  testami i recovery po `appRestoredResult`. Własny kod jest dopuszczalny dopiero,
  gdy plugin nie pokrywa kontraktu lub zwiększa ryzyko danych/UX.
