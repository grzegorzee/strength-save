...
type UserSort = 'activity' | 'recent' | 'name';
...
const AdminDashboard = () => {
  ...
  const filteredUsers = useMemo(() => {
    ...
    // Z98: domyślnie po aktywności z rollupu (fallback lastLogin w sortUsersByActivity).
    if (userSort === 'activity') return sortUsersByActivity(matched);
    return matched.sort((a, b) => userSort === 'name'
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent className="p-0">
          <UsersActivityTable
            ...
            onToggleExpand={setExpandedUid}
            detailsByUser={detailsByUser}
            loadingDetails={loadingDetails}
            onToggleAccess={(uid, enabled) => void toggleAccess(uid, enabled)}
            onToggleFeature={(uid, feature, enabled) => void toggleFeature(uid, feature, enabled)}
            onToggleSuspended={(uid, suspended) => void toggleSuspended(uid, suspended)}
            onSendEmail={(uid) => void handleSendEmail(uid)}
            onResendCode={(uid) => void handleResendCode(uid)}
