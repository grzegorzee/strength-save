...
import { buildCanonicalState, buildCycleChoice as buildCycleChoiceBase } from '@/test/canonical-states';
import type { PlanCycleChoice } from '@/types/cycles';
import type { TrainingDay } from '@/data/trainingPlan';

/** Adapter do wspolnego buildera WP-6 (entry, chosenAt, overrides): dni z planu,
 *  bez domyslnej nazwy (test "planName > szablon > wlasny" liczy na jej brak),
 *  klucze nadpisane `undefined` nie powstaja (kontrakt Firestore). */
const buildCycleChoice = (startDate: string, days: TrainingDay[], overrides: Partial<PlanCycleChoice> = {}): PlanCycleChoice => {
  const { entry = 'onboarding', version: _v, chosenAt: _c, ...rest } = overrides;
  const built = buildCycleChoiceBase(entry, `${startDate}T06:00:00.000Z`, {
    daysPerWeek: days.length,
    trainingDays: days.map((day) => day.weekday),
    ...rest,
  });
  if (!('planName' in overrides)) delete built.planName;
  for (const key of Object.keys(overrides) as (keyof PlanCycleChoice)[]) {
    if (overrides[key] === undefined) delete built[key];
  }
  return built;
};
