...
- [ ] **Z210: sandbox/TestFlight + Play Internal.** Na obu platformach: eligible monthly 7 dni, eligible yearly 14 dni, wykorzystany trial, `unknown`, purchase, cancel, restore, offline/error. Zweryfikuj teksty regulaminu, privacy, auto-renew i brak mylacej obietnicy. Po zakupie/restore stan `pro` ma dojsc do web, Apple Watch i sparowanego Garmina bez ponownego zakupu; wygasniecie nie moze usunac danych ani niewyslanej sesji. **Stan:** przygotowanie kodowe zamkniete przez Z208+Z209 (trial copy tylko przy eligible, `unknown`/`ineligible` bez obietnicy, restore i offline/error z osobnymi sciezkami, ceny/oszczednosc z realnych cen sklepu); propagacja `pro` na web/Watch/Garmin idzie przez RC webhook -> `users/{uid}.subscription`, ktory wszystkie powierzchnie czytaja serwerowo (Z226/Z227), a wygasniecie triala nie usuwa danych ani kolejki (Z228: expired vs revoked). **KROK USERA:** pelna macierz sandbox wymaga realnego iPhone z buildem 84+ z TestFlight (konto sandbox App Store) oraz Androida z AAB 6+ z Play Internal (po 7 krokach z `docs/X25-MONETIZATION-STATUS.md`); scenariusze: eligible 7/14, wykorzystany trial, unknown (offline), purchase, cancel, restore, expiry przy niewyslanej sesji + weryfikacja, ze `pro` dociera do web/Watch/Garmin bez drugiego zakupu. Checkbox otwarty do wykonania obu macierzy.
...
- [x] **Z211: batching telemetrii.** Flush z 30 s na 5 min, a dodatkowo przy `online`, przejsciu aplikacji w tlo i zamknieciu tam, gdzie platforma pozwala. Bufor localStorage i retry pozostaja. Test fake timers: maks. 12 okresowych flushy/h zamiast 120. **Dowod:** TDD: `src/test/telemetry-heartbeat.test.tsx` RED (121 flushy/h, brak pagehide, flush przy powrocie z tla), po zmianie `TelemetryHeartbeat` 3/3 PASS: interval 5*60_000 daje 1+12 wywolan/h na fake timers, `online` i `pagehide` flushuja, `visibilitychange` flushuje tylko przy `document.hidden` (powrot do foreground bez flusha, bo JS byl wstrzymany i nic sie nie zebralo). Bufor localStorage i retry w `app-telemetry.ts` nietkniete. Pelna suita 157 plikow/1298 PASS, typecheck, lint PASS.
- [x] **Z212: deduplikacja push registration.** Lokalnie przechowuj hash tokenu, uid i czas potwierdzenia. Backend wywoluj tylko po zmianie tokenu/uid albo po 30 dniach. Event refresh tokenu musi natychmiast rejestrowac nowy token. Logout usuwa stan poprzedniego uid. **Dowod:** TDD: `src/test/push-registration-dedup.test.ts` RED (brak modulu), po implementacji 7/7 PASS. `push-registration-state.ts`: SHA-256 hash tokenu (fallback FNV bez WebCrypto), `shouldRegisterPushToken` (brak stanu / inny hash / inny uid / >=30 dni = rejestruj, inaczej skip), round-trip localStorage z guardem na uszkodzony JSON. `push-notifications.ts`: `saveToken(uid, token, {force})` z dedupem przed callable i `markPushTokenConfirmed` po sukcesie; refresh event rejestruje nowy token natychmiast (inny hash), ten sam token z iOS `tokenReceived` przy kazdym getToken = zero callable; `requestPushPermission` z Ustawien ma `force` (wyjscie z kazdego stanu bez czekania 30 dni); `unregisterPushForUser` czysci stan we wszystkich sciezkach logout. Pelna suita 158 plikow/1305 PASS, typecheck, lint PASS.
- [x] **Z213: pomiary i cykle per ekran.** Dashboard pobiera najnowszy pomiar (`limit(1)`) i aktywny cykl. Pelne pomiary/cykle pozostaja na swoich ekranach, z paginacja gdzie potrzebna. Nie zmieniaj wynikow UI. **Dowod:** TDD: `src/test/measurement-read-tiers.test.ts` RED, po implementacji 8/8 PASS + stare 4/4 store PASS. Tier pomiarow w `workout-read-store` ('none' 0 / 'latest' sonda 25 / 'full' 365; effective = max aktywnych subskrybentow, restart listenera przy zmianie); sonda zamiast literalnego `limit(1)`, bo selekcja najnowszego pomiaru pomija puste/czesciowe wpisy (`selectLatestMeasurement`, jedna implementacja dla obu sciezek, test rownowaznosci sonda==pelna lista). Konsumenci: 'latest' Dashboard/WorkoutDay/ExerciseProgressionDialog, 'none' 12 komponentow sync/list bez pomiarow, 'full' Pomiary/Analityka/Ustawienia (eksport). **Cykle: udokumentowana roznica wobec checkboxa** — Dashboard i WorkoutDay realnie potrzebuja PELNEJ listy cykli (resolver nazw historycznych treningow `buildWorkoutResolver`, sekcja visibleCycles), wiec active-only zmieniloby wyniki UI, czego zadanie wprost zakazuje; listener cykli to max 60 malych dokumentow (koszt marginalny vs 365 pomiarow), a deduplikacja 13 osobnych listenerow cykli w jeden store to zakres Z215/Z216. Bramki: 159 plikow/1313 PASS, typecheck, lint, build, build:mobile, dist smoke, bundle 1 272 855 B PASS, pelne E2E 388/388 PASS (naprawione przy okazji: paywall-funnel po copy Z208, pre-istniejacy webkit fail analytics-pdf przez natywny navigator.share).
- [x] **Z214: aktywnosci per ekran.** Dashboard pobiera ograniczone ostatnie okno potrzebne do kart. Pelna historia Strava/manual jest paginowana na widokach historii/analityki. Porownaj wyniki kart przed/po na fixture z historia >500 rekordow. **Dowod:** `src/test/activity-window.test.ts` 4/4 PASS na fixture 600+ rekordow: `weeklyStravaKm` i `currentWeekCardio` (logika kart wyciagnieta 1:1 z Dashboardu do `src/lib/activity-window.ts`) daja identyczne wyniki na pelnej historii i na oknie. Hooki `useStrava`/`useManualActivities`/`useActivities` przyjmuja `sinceDate` (`where('date','>=')` na istniejacym indeksie userId+date, limit 500 zostaje jako bezpiecznik). Konsumenci: Dashboard okno od poniedzialku biezacego tygodnia planu (wszystkie 3 uzycia: weeklyKm, HybridWeekStrip, timeline licza wylacznie biezacy tydzien; `computeDailyLoads` agreguje per dzien, bez rolling window), DayPlan okno od dzis (pokazuje tylko dzisiejsze cardio). Widoki historii/analityki (StravaTab, AnalyticsWeeklyTab 12 tyg, HybridLoadCard, TrainingPlan kalendarz) zostaja na pelnym limicie 500 zgodnie z checkboxem (pelna historia na widokach historii/analityki; ich okna siegaja wielu tygodni). Bramki: 160 plikow/1317 PASS, typecheck, lint, build, build:mobile, dist smoke, bundle, pelne E2E 388/388 PASS.
...
- [x] **Z215: mapa zaleznosci od calej historii.** Wypisz wszystkie komponenty liczace streak, PR, objetosc, wykresy i poprzedni ciezar. Dla kazdego okresl recent realtime, paginowana historia albo agregat. Nie implementuj, dopoki testy fixture nie zamroza obecnych wynikow. **Dowod:** `docs/Z215-HISTORY-DEPENDENCY-MAP.md` — 17 obliczen sklasyfikowanych (RECENT / RANGE / AGGREGATE / GOTOWE) z konsumentami i przypisanymi testami fixture; kluczowe odkrycia: przy >500 treningach obecne liczby all-time sa JUZ liczone z okna 500 (agregat Z217 je naprawia, nie tylko przyspiesza), currentStreak liczy tygodnie od realnego dzis (dryf na stalym fixture), a 13 osobnych listenerow usePlanCycles to kandydat na wspolny store w Z216. Zamrozenie: istniejace suity (all-time-stats, summary-utils, history-stats, monthly-stats, pr-utils, hybrid-load, cycle-insights) + NOWY `src/test/z215-history-freeze.test.ts` 3/3 PASS — golden values na deterministycznym fixture 600 treningow (tonaz 374400, licznik 540, serie 1080, czas, ulubione 150, pierwsza data, longestStreak 86); Z217 ma liczyc rownowaznosc agregatu przeciw tym wartosciom. Zadnych zmian w kodzie produkcyjnym.
- [x] **Z216: recent realtime + pagination.** Globalny listener nie moze zawsze pobierac 500 treningow i 365 pomiarow. Ostatnie treningi zostaja realtime; starsze sa pobierane kursorem na zadanie. `AutoSyncOnReconnect` ma synchronizowac kolejke/draft bez utrzymywania szerokiego listenera na kazdym ekranie. **Dowod:** TDD: 4 nowe testy tierow workouts RED -> `measurement-read-tiers.test.ts` 12/12 PASS. Tier treningow w store ('recent' 120 / 'full' 500, effective=max, restart przy zmianie); pomiary juz tierowane (Z213: 365 -> 25/0). Krytyczne znalezisko: globalny `AppHeader` trzymal FULL listener (500 treningow + 365 pomiarow) NA KAZDYM EKRANIE przez bezposrednie `useFirebaseWorkoutReads(uid)` — teraz 'none'+'recent'. Konsumenci 'recent' (9): Dashboard, DayPlan, Profile, TrainingPlan, AppHeader, AutoSyncOnReconnect, SyncCenterCard, WorkoutImportWizard, AnalyticsWeeklyTab, HybridLoadCard (okna 12-tyg mieszcza sie w 120); 'full' zostaje na ekranach liczacych z pelnej historii (WorkoutDay z PR baseline — trening i tak trzyma listener przez sesje, Achievements, Analytics/ChartsTab, Cycles/NewPlan backfill, Settings eksport, StravaTab, Measurements, ExerciseProgressionDialog). Liczby all-time: kafle/licznik naglowka/Profile z agregatu Z217; STREAK Dashboardu liczony ta sama funkcja `calculateStreakDetails` z dat mapy wkladow agregatu (`sanitizeAggregate.completedDates`) — okno 120 przycinaloby dlugie serie (40+ tygodni); fallback z okna tylko bez agregatu. Starsze na zadanie: `AllTimeStatsSheet` przy otwarciu dociaga PELNA historie kursorem (`fetchWorkoutRange`, baza z okna renderuje natychmiast, blad sieci = zostaje baza), WorkoutHistory ma kursor od X10. Lazy backfill agregatu: klient przy potwierdzonym braku dokumentu wola `rebuildWorkoutAggregate` (guard 1x/dzien). Swiadomie pominiete: dedup 13 listenerow usePlanCycles (60 malych doc, marginalny koszt — odnotowane w mapie Z215), kafel "ostatni PR" Dashboardu pozostaje heurystyka okna (byl nia juz przy 500). Bramki: app 162 plikow/1330 PASS, typecheck, lint, build, build:mobile, dist smoke, bundle, E2E 387/388 + 1 flaky webkit PASS w izolacji.
- [x] **Z217: agregaty statystyk.** Jezeli Dashboard potrzebuje all-time, dodaj wersjonowany dokument agregatu aktualizowany idempotentnie przy finalizacji/edycji/usunieciu treningu. Przygotuj bezpieczny, wznawialny backfill i test rownowaznosci agregatu z obecnymi obliczeniami. Brak agregatu musi miec bezpieczny fallback. **Dowod:** TDD: `functions/src/workout-aggregate.test.ts` RED -> 7/7 PASS, w tym test ROWNOWAZNOSCI przeciw golden Z215 (fixture 600 -> 374400 kg / 540 treningow / 1080 serii / 540*3600 s / 2024-12-11). Architektura: `users/{uid}/aggregates/allTime` (schemaVersion 1) trzyma MAPE WKLADOW per workoutId — totals przeliczane z mapy, wiec dokument jest odtwarzalny, idempotentny (set po kluczu, nie increment) i odporny na at-least-once delivery; trigger `onWorkoutWrittenAggregate` (workouts/{id}) robi transakcyjny apply z no-op gdy wklad bez zmian, a przy braku dokumentu albo starym schemacie wykonuje PELNY rebuild historii usera (istniejacy user nigdy nie dostanie agregatu z jednego treningu); backfill = callable `rebuildWorkoutAggregate` (idempotentny pelny rebuild wlasnego konta, paginacja 500). Rules: aggregates read owner/admin, write tylko backend (+4 testy, rules 174/174 PASS w emulatorze JDK21). Klient: `useWorkoutAggregate` + `sanitizeAggregateTotals` (zly ksztalt = null), kafle Dashboardu (tonaz, licznik) uzywaja agregatu z fallbackiem na dotychczasowe liczenie z okna; AllTimeStatsSheet zostaje na obecnym zrodle do Z216 (v1 agregatu swiadomie bez streakow/PR/ulubionego — semantyka tygodniowa i nazwy cwiczen w mapie to Z216+). Wdrozone: rules deploy PASS, `firebase functions:list` pokazuje obie funkcje ACTIVE v2/nodejs22 (pierwszy deploy triggera padl na propagacji Eventarc, drugi Successful update). Bramki: app 162 plikow/1323 PASS, functions 194 PASS, typecheck, lint, build, build:mobile, dist smoke, bundle PASS.
